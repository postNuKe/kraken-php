(function(jQuery) {
	...
	jQuery.fn.stickyNotes.prepareContainer = function(container) {
		...
		if (jQuery.fn.stickyNotes.options.controls) {
			jQuery("#sticky-container").html('<button id="add_note">A&ntilde;adir Nota</button>');			
	...
	jQuery.fn.stickyNotes.createNote = function() {
		var pos_x = 0;
		var pos_y = 0;		
		var note_id = 1;//jQuery.fn.stickyNotes.notes.length + 1;
		if(jQuery.fn.stickyNotes.notes.length == 0){
			note_id = 1;
		}else{
			note_id = jQuery.fn.stickyNotes.notes.slice(-1)[0]['id'] + 1;
		}
		//alert(jQuery.fn.stickyNotes.notes.length);
		//var note_id = jQuery.fn.stickyNotes.notes.slice(-1)[0]['id'] + 1;
		//alert(note_id);
    ...
    jQuery.fn.stickyNotes.createNoteBackground = function() {
        ...
        if (jQuery.browser.msie && jQuery.browser.version <= 6)  {
            background = $(document.createElement('div')).addClass("background").html('<img src="images/layout/sticky/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'images/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
        } else {
            background = $(document.createElement('div')).addClass("background").html('<img src="images/layout/sticky/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
        }
        return background;
